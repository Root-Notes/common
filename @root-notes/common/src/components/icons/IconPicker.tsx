      ...
      data.iconNames
        .filter(
          (name) =>
            ...
            search.toLocaleLowerCase().includes(name.toLocaleLowerCase()) ||
            startCase(name)
              .split(" ")
              .slice(1)
              .join(" ")
    ...
    <Tabs.Panel value={family} p="md">
      <Stack gap="md" className="picker-tab-panel">
        <TextInput
          ...
          label={t("common.components.modals.iconPicker.panel.search")}
        />
        ...
        <Group gap="sm" className="controls">
          ...
          <Select
            ...
            label={t("common.components.modals.iconPicker.panel.pageSize")}
          />
        </Group>
      </Stack>
    </Tabs.Panel>
...
function IconPickerModal({
  ...
  return (
    <Modal
      ...
      size="xl"
      title={t("common.components.modals.iconPicker.title")}
    >
  ...
  const tabData: TabData[] = useMemo(
    () =>
      Object.entries(iconTypes)
        .filter(([family]) =>
          (families ?? ["md", "tb", "gi", "di"]).includes(family as any)
        )
        .map(([family, icons]) => ({
      ...
      <Stack gap="sm" className="custom-input icon-picker">
        ...
        >
          <Group gap="sm" justify="space-between">
            <GenericIcon family={val.family} name={val.name} size={32} />
